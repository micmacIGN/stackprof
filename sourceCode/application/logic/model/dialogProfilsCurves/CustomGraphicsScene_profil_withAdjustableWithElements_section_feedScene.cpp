...
bool CustomGraphicsScene_profil::feedScene(
    ...
    qDebug() << __FUNCTION__ << "(CustomGraphicsScene_profil)" <<  "with xGraphMin =" << xGraphMin;
    qDebug() << __FUNCTION__ << "(CustomGraphicsScene_profil)" <<  "with xGraphMax =" << xGraphMax;
    ...
    if (!leftRightSides_linearRegressionModelPtr) {
        qDebug() << __FUNCTION__  << " error: getting ptr on  measureIndex :  (_bOnMainMeasure = " << stackedProfilInEdition._measureIndex._bOnMainMeasure;
        if (!stackedProfilInEdition._measureIndex._bOnMainMeasure) {
            qDebug() << __FUNCTION__  << " error: getting ptr on  measureIndex :  (_bOnMainMeasure = " << stackedProfilInEdition._measureIndex._secondaryMeasureIdx;
        }
        return(false);
    }
    ...
    if (!leftRightSides_linearRegressionModelPtr->_modelValuesResults._bComputed) {

        qDebug() << __FUNCTION__
                 << "leftRightSides_linearRegressionModelPtr->_modelValuesResults._bComputed "
                 <<  leftRightSides_linearRegressionModelPtr->_modelValuesResults._bComputed;
    }
    ...
    if (bReport) {
        ...
        qDebug() << __FUNCTION__ << "compute_pointsCoupleForLineRegDisplay_left_right_sides said false";
    }
    ...
    _cccs_yMin_whenAlone = yGraphMin;
    _cccs_yMax_whenAlone = yGraphMax;
    ...
    for (auto &v: _computedValuesWithValidityFlag) {
        if (v._anonymMajorMinorWithValidityFlag._bValuesValid) {
            ...
            v._anonymMajorMinorWithValidityFlag._major_centralValue =
                        cccsY_to_qscY(v._anonymMajorMinorWithValidityFlag._major_centralValue);// + (-yGraphMin));
    ...
    if (!_CGItemDblSidesXAdjWithCompLines_mainMeasure) {
        ...
        _CGItemDblSidesXAdjWithCompLines_mainMeasure = new CustomGraphicsItemDoubleSidesXAdjustersWithComputedLine(
           ...
           //only this parameter is about final pixel size here:
           {_pixel_gripSquareSideWidth,
            _pixel_gripSquareSideHeight}//_parametersForPixelResizeInvariance
            );
        ...
        qDebug() << __FUNCTION__  << "3) before #522";
        //direct forward:
        ...
        qDebug() << __FUNCTION__  << "3) after #526";
